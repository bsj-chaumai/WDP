...
- **お申込みと規約のご確認**
  ご確認とやりとりが発生する工程です。現在の流れは調査を進めている段階です。
- **結果の分析と提案書への反映**
...
## 1-4. 課題3：改修前後の効果が示せず、継続と有料化に繋がらない

**Before 83件に対し、After 5件。**

Afterは現時点では無償でご提供いただいており、
対象者への自動再配信の仕組みも備わっていると伺っております。
それにもかかわらず、実施が積み上がっていない状況です。

**根本的な原因は、改修の前後で「何がどう変わったのか」を
示せていないことにあると考えています。**

現在、実際に残るのは
「**空間がきれいになった**」「**改修が終わった**」という体感です。
...
改修前後の効果が示せていない
  → 継続して測る理由が生まれない
  → 有料のサービスとして提案する根拠と説得力が揃わない
```

この背景として、次の点があると考えています。
...
| **結果の分析と提案書への反映** | 回答データを自動で集約し、傾向を確認できる状態にします。<br>そのうえで、採用する内容を選んで出力できるようにします |

あわせて、ナーチャリングなど**早い段階で気軽に実施できる状態**を整えます。

#### お申込み・規約について — 確認とやりとりの時間を削減する

**お申込みと規約の確認をWDP上の一つの流れにまとめることで、
コンサルタントとエンドクライアントとの間で発生している
確認・やりとりの時間を削減できると考えています。**

なお、現在のお申込みと規約確認の流れについては調査を進めている段階であり、
確認が必要な規約の内容もこれから伺う予定です。
そのため、**削減できる範囲は調査と確認の結果を踏まえて確定**させたいと考えております。

そのうえで、次の効果を期待しています。

| 期待できること | 内容 |
|---|---|
| **確認のやりとりが減る** | 案件ごとに発生している確認の往復を、画面上の手続きに置き換えます |
| **手続きが一箇所にまとまる** | お申込みと規約の確認を別々に進める必要がなくなります |
| **進捗が確認できる** | どこまで完了しているかが分かるため、状況を確認する手間が減ります |
| **早い段階から実施しやすくなる** | 手続きが軽くなることで、ナーチャリング段階での実施が現実的になります |

#### 結果の分析について — 順番に進める形から、並行して進める形へ

現在は、結果を取得した後に内容を確認し、
数値から文章へ変換して提案に反映する、という順番で進められていると伺っております。

回答データを自動で集約できれば、
**回答が集まっている途中でも傾向が見えます。**
すべて揃うのを待たずに検討を始められるため、
分析が回収と並行して進む形になります。

これは「分析が速くなる」以上に、**全体の期間を短縮します。**

| 改善できること | 内容 |
|---|---|
| **回答の回収と並行して検討を始められる** | 全件揃うのを待たずに傾向が確認できます |
| **集約された内容から作業を始められる** | 数値から文章に変換する作業が減ります |
| **担当者による差が縮まる** | 全員が同じ集約内容から始めるため、読み解きの属人性が下がります |
| **見落としやすい情報が漏れなく上がる** | 自由記述などは読み飛ばされやすいですが、集約されることで必ず確認できます |

#### 事業面での効果

この課題の改善は、他の課題の前提にもなります。

```
提案期間内に収まる
  → 提案期間中にサーベイを実施できる案件が増える
  → データに基づいた提案ができる案件が増える（受注率への貢献）
  → Before実施件数が増える → Afterの母数も増える
```

**あわせて、機会損失が減ります。**
現在は、サーベイが期間内に間に合わない場合、
提案はデータなしで出ることになり、その案件では受注率への貢献が得られません。
リードタイムの短縮は、**そうした案件を減らすこと**にもなります。

#### 効果の測り方

| 指標 | 何を見るか |
|---|---|
| お申込みから実施までの日数 | お申込み・規約の改善効果 |
| 結果取得から提案反映までの日数 | 分析の改善効果 |
| **提案期間内にサーベイを実施できた案件の割合** | この課題全体の指標 |
| ナーチャリング段階での実施件数 | 手続きを軽くしたことの効果 |
| サーベイ結果を根拠として含めた提案の件数 | 受注率への繋がり |

### 課題3：改修前後の効果が示せず、継続と有料化に繋がらない
...
```
改修前後の効果が示せていない（残るのは体感のみ）
  ↓
継続して測る理由が生まれない（使用継続の条件がない）
  ↓
有料のサービスとして提案する根拠と説得力が揃わない
```

回答が集まらないことも、この連鎖の中にあると考えています。
**確かめたいものがない状態では、回答する理由も生まれません。**
...
| | 内容 | 扱い |
|---|---|---|
| **設問そのもの** | 設問の文言、選択肢、尺度 | **変更しません** |
| **配信する範囲と追加分** | どの設問を配信するか／どの設問を加えるか | 案件の目標に応じて決めます |

**集計ロジックを守るのは前者です。**
各設問のスコアはその設問への回答から算出されるため、
設問自体を変更しなければ、算出方法はそのまま有効です。

つまり本提案は、**既存の設問には一切手を入れず、
「どれを配信するか」と「何を加えるか」を案件ごとに決める**という考え方です。

**共通コアを固定する理由：**
案件によって配信範囲が変わる中でも、
他社との比較や推移のグラフが成立する部分を残しておく必要があります。
そのため、属性・働く場所の割合・全体満足度・eNPSは共通コアとして固定します。

#### Afterでどこまでの比較が必要かについて

事業計画には次の記載があります。

> 52問あるWDSの中から最も関連性の高い**項目を3つ選択**し、
> 現状に対して目標のスコアを設定します

XDでは**設問単位**で目標を設定されていると理解しております。
その場合、Afterで該当の設問をそのまま配信すれば、
Beforeとの比較は設問単位で成立します。

一方、6項目のスコアやワークデザインスコアを算出するには、
その項目に属する設問すべてが必要になります。

Afterでどこまでの比較が必要かによって、配信する設問数が変わります。

| Afterで必要とされるもの | 配信が必要な設問 | 設問数 |
|---|---|---|
| 目標に設定した設問の比較 | 該当の設問のみ | 2〜3問 |
| ＋ 該当項目のスコア | その項目に属する設問すべて | 8〜18問程度 |
| ＋ ワークデザインスコア | 全設問 | 52問 |

現在Afterで**どこまでを使われているか**を確認させていただき、
必要な範囲に合わせて構成を決めたいと考えております（2-9 参照）。

なお、パルス版の設問はすでに全設問の一部を用いた構成と理解しており、
一部の設問で測るという考え方自体は、すでに運用されているものと認識しております。
...
| **Afterでどこまでの比較を使われているか** — 目標設問の比較のみか、項目スコアまでか、ワークデザインスコアまでか | **配信する設問数が大きく変わります。**最も確認したい点です |
| **6項目のスコアの算出方法** — どの設問がどの項目に対応しているか | 配信範囲と共通コアを決めるために必要です |
...
本提案では、次の方針とさせていただきたいと考えております。

> **既存の設問（文言・選択肢・尺度）は一切変更しません。**
> **案件ごとに決めるのは、どの設問を配信するかと、何を加えるかです。**

設問自体を変更すると、これまで蓄積したスコアとの比較ができなくなります。
文言のわずかな修正や尺度の変更も、気づかないうちに比較を崩す要因になります。
そのため、変更ではなく**配信範囲と追加**で対応する形をご提案しています。
...
課題の根本は、改修前後の効果が示せていないことにあると認識しております。
ただしその手前で、**回答が集まらなければ示す材料そのものがありません。**
...
> **Không sửa câu hỏi nào đang có. Quyết định theo mục tiêu dự án là: gửi những câu nào, và thêm những câu gì.**
...
**Cái bảo vệ logic tính toán là phần bất biến** — điểm của một mục tính từ câu trả lời của chính câu hỏi đó, nên miễn câu hỏi không đổi thì cách tính vẫn đúng.

Cách nói này chặt hơn "giữ bộ chuẩn" vì nó **cấm cả việc sửa nhẹ chữ hay đổi thang đo** — thứ dễ bị bỏ qua nhưng làm hỏng so sánh mà không ai nhận ra.
...
事業計画 viết: 「52問あるWDSの中から最も関連性の高い**項目を3つ選択**し」— chọn **3 câu hỏi** trong 52, không phải 3 trục. Bản MVP thì 2 câu.

Nghĩa là **cách so sánh của chính XD đã làm việc ở cấp câu hỏi**. Nếu After giữ nguyên 3 câu đó, so sánh hợp lệ — **không cần tính lại điểm trục**.
...
| + điểm tổng ワークデザインスコア | Toàn bộ 52 câu | 52 |

Ba mức khác nhau **rất nhiều** về gánh nặng trả lời. Chưa biết Vis đang dùng tới mức nào ở After — đã đưa lên **đầu danh sách xác nhận** ở 2-9.

### Tiền lệ hỗ trợ: pulse 19 câu đã là tập con

Bộ pulse Vis tự tạo tháng 1/2025 **chính là một tập con của 52 câu**. Nghĩa là họ **đã chấp nhận nguyên tắc đo bằng tập con** — nên đề xuất gửi tập con ở After không phải ý lạ. Đây là lập luận tốt để dùng khi trình bày.
...
| 10 | Luồng お申込み・規約確認 hiện tại — gồm những bước nào, ai xác nhận, mất bao lâu | Biết F1 cắt được bao nhiêu | Vis |
