...
#### お申込み・規約について — 最も大きいのは「日数が読めること」

現在は書面のやり取りとリーガルチェックが入るため、
日数がクライアント側や法務のご都合に左右されます。

3〜4週間という枠の中では、**遅いことよりも読めないことが進行を止めます。**
読めなければスケジュールに組み込めません。

| 改善できること | 内容 |
|---|---|
| **お申込みから実施までの日数が読めるようになる** | 待ち時間が発生する箇所が減り、提案スケジュールに組み込めるようになります |
| **案件ごとのリーガルチェックが不要になる可能性** | 規約を標準化し一度ご確認いただければ、案件ごとの確認が不要になります。確認が必要な規約の内容を伺ったうえで、削減できる範囲を確定したいと考えています |
| **手続きの状況が見える** | 書類がどこにあるかを確認する必要がなくなります |
| **早い段階から実施できるようになる** | 手続きが軽くなることで、ナーチャリング段階での実施が現実的になります |

#### 結果の分析について — 順番に進める形から、並行して進める形へ

現在は、回答が揃うのを待つ → 数値を読む → 文章に起こす →
提案に反映する、という順番になっています。
